...
#load("~/Bioconductor/Projects/GraphCombine/MetaBreast/data/vantVeer.RData") 
...
't-statistic' for the difference between lymph node positive and negative
...
The procedure procedure described above is also implemented in the function
...
We get a matrix in the following form. zScores for the two splits
('zSco\_Ex\_1' and 'zSco\_Ex\_2') for the original set ('zSco\_Ex\_3'), and for the
combined set ('zSco'). We furthermore get the other values we calculated above.
...
We plot the zScores of original data set against the zScores of the
...
see that we really gain much by combing the sets. 
<<IDRplot,fig=TRUE,results=hide>>=
...
ScoresFDR <- zScoreFDR(esets, classes,useREM=FALSE, numberOfPermutations=20,
                         CombineExp=1:2)
@
...
tow sided situation. Each slot contains a matrix with the values from zScores and additional FDR's
...
combined set is red and for the result for the original set is blue. We
extract the FDR for the two sided case (taking the absolute values of the z
scores). It can be see that the combined data set has a lower FDR than the
splits but a higher FDR that the original set. 
...
##100 and 500 
#for (k in c(100,500)){
#plot(FDRwholeSettwo,pch="*",col="red",ylab="FDR",xlab="Number of genes",xlim=range(0,k))
#for(j in 1:3)
...
Similar to the \Rfunction{IDRplot}
it shows the following: for each study (indicated by different colors) and various thresholds for the FDR (x axis) the number of
genes that are below this threshold in the given study but above in all other
studies are shown (y axis). The experiment colors for this plot are the same
as in the plots before. Here again we exclude the original experiment (like in
the IDR plot).

<<theFDRplots,results=hide,fig=TRUE>>=
par(mfrow=c(2,2))
CountPlot(ScoresFDR,Score="FDR",kindof="neg",cols=c("red",theNewC),
	main="Negative FDR", xlab="FDR threshold", ylab="Number of genes",Combine=1:2)
...
CountPlot(ScoresFDR,Score="FDR",kindof="two.sided",cols=c("red",theNewC),main="two sided FDR", xlab="FDR threshold",ylab="Number of genes",Combine=1:2)
@
